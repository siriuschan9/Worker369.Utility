...
public class IPv6Subnet : IComparable, IComparable<IPv6Subnet>, IEquatable<IPv6Subnet>
{
    // The network address defines this subnet.
    protected IPv6Address _network_address;

    public string CIDR => _network_address.ToString(IPv6AddressDisplay.IP_PrefixLength);

    public IPv6Address FirstIP => _network_address.Clone();

    public IPv6Address LastIP
    {
        get
        {
            byte[] my_bytes     = _network_address.Bytes;
            byte[] subnet_mask  = _network_address.PrefixMask.Bytes;
            ...
            return new IPv6Address(new_bytes, _network_address.PrefixLength);
        }
    }

    public IPv6PrefixMask PrefixMask => _network_address.PrefixMask;

    public NumberInfo Size => _network_address.PrefixMask.Size;
    ...
    internal IPv6Subnet(byte[] octets, int prefixLength)
    {
        _network_address = new IPv6Address(octets, prefixLength).NetworkId;
    }

    public bool ContainsIpAddress(IPv6Address ip)
    {
        var clone = ip.Clone();
    ...
    public bool ContainsIpAddress(string ip) => ContainsIpAddress(IPv6Address.Parse(ip));

    /// <summary>
    /// Returns the next adjacent subnet.
    /// </summary>
    /// <param name="n">The number subnets to return.</param>
    public IEnumerable<IPv6Subnet> Next(int n = 1)
    {
        if (n < 1)
        ...
        var increment_value = (byte)(1 << ((128 - prefix_length) % 8)); // increment size within the byte.
        ...
        while (returned < n)
        {
            ...
            yield return new IPv6Subnet(bytes, prefix_length); returned++;
        }
    }
    ...
    /// </summary>
    /// <param name="n">The number subnets to return.</param>
    public IEnumerable<IPv6Subnet> Previous(int n = 1)
    {
        if (n < 1)
            throw new ArgumentException("The parameter must be greater than 0", nameof(n));
        ...
        var decrement_value = (byte)(1 << ((128 - prefix_length) % 8)); // decrement size within the byte.
    ...
    public override string ToString() =>_network_address.ToString(IPv6AddressDisplay.IP_PrefixLength);

    public int CompareTo(object obj)
    {
        if (obj is null) return 1; // Follow convention: non-null > null

        if (obj is not IPv6Subnet other)
            throw new ArgumentException($"Object is not {typeof(IPv6Subnet)}");
        else
            return CompareTo(other);
    }

    public int CompareTo(IPv6Subnet other)
    {
    ...
    public bool Equals(IPv6Subnet other)
    {
    ...
    public override bool Equals(object obj) => Equals(obj as IPv6Subnet);

    public override int GetHashCode() => _network_address.GetHashCode();

    public static bool IsOverlapping(IPv6Subnet a, IPv6Subnet b)
    {
        ...
        for (int i = 0; i < 16 && i * 8 < compare_prefix; i++)
        {
    ...
    public static IEnumerable<IPv6Subnet> Split(IPv6Subnet parent, int bits)
    {
        int parent_prefix_length = parent.PrefixLength;
        int child_prefix_length  = parent_prefix_length + bits;

        if (bits < 0 || bits > 128)    throw new ArgumentException("Bits must be between 0 and 128", nameof(bits));
        if (child_prefix_length > 128) throw new OverflowException("Network bits of child subnets exceeded 128.");

        byte[] parent_bytes = parent._network_address.Bytes;

        BigInteger count = BigInteger.One << bits ;   // Number of child subnets to make.
        int shift_bits   = 128 - child_prefix_length; // Shift child differentiator bits unit the prefix boundary.

        for (BigInteger i = BigInteger.Zero; i < count; i++) // Number of child subnets.
        {
            // Initialize child bytes.
            byte[] child_bytes = GetBytes(i);
            ...
            yield return new IPv6Subnet(child_bytes, child_prefix_length);
        }
    }

    public static IPv6Subnet Merge(IPv6Subnet[] subnets)
    {
        // Short-circuit.
        ...
        // Initialize mask to track common bits. Default values - all 0.
        var mask_bytes = new byte[16];

        int i = 0, j = 0;

        for (i = 0; i < 16; i++)          // Byte index counter
        {
            for (j = 0; j < 8; j++)       // Bit index counter
            {
                ...
                {
                    // Initialize byte array for the new summarized subnet.
                    byte[] common_bytes = new byte[16];

                    // Extract out common network bits using the mask.
                    for (int k = 0; k < 16; k++)
                        common_bytes[k] = (byte)(first_id[k] & mask_bytes[k]);

                    return new IPv6Subnet(common_bytes, i * 8 + j % 8);
                }
            }
        }

        Debug.Assert(mask_bytes.All(octet => octet == 0)); // No common network bits - mask must be all 0.
        Debug.Assert(i * 8 + j % 8 == 128);                // No common network bits - all bits must be walked.

        return new IPv6Subnet(new byte[16], 0);            // No common network bits - return ::/0
    }

    public static IPv6Subnet Parse(string cidr)
    {
        if (TryParse(cidr, out IPv6Subnet subnet))
            return subnet;

        throw new ArgumentException("CIDR is not valid");
    }

    public static bool TryParse(string cidr, out IPv6Subnet subnet)
    {
        if (Regex.IsMatch(cidr, IPPattern.IPv6Address))
            cidr += "/128";

        if (Regex.IsMatch(cidr, IPPattern.IPv6Subnet))
        {
            string[] parts         = cidr.Split('/');
            string   prefix        = parts[0];
            int      prefix_length = Convert.ToInt32(parts[1]);

            var ip_address = System.Net.IPAddress.Parse(prefix);
            var bytes      = ip_address.GetAddressBytes();

            subnet = new IPv6Subnet(bytes, prefix_length);
            return true;
        }
    ...
    public static IPv6Subnet GetRandom()
    {
        var rand          = new Random();
        int prefix_length = rand.Next(129);

        BigInteger value = BigInteger.Zero;

        for (int i = 127; i >= 127 - prefix_length; i--) // Randomize network bits only.
            value |= (BigInteger)rand.Next(2) << i;

        return new IPv6Subnet(GetBytes(value), prefix_length);
    }

    public static bool operator ==(IPv6Subnet a, IPv6Subnet b) => a.Equals(b);
    ...
    protected static byte[] GetBytes(BigInteger value)
    {
        ...
        for (int i = 0, j = 120; i < 16; i++, j -= 8)
            bytes[i] = (byte)((value >> j) & 0xff);   // bytes[0]>>120, bytes[1]>>112, ..., bytes[15]>>0
    ...
    {
        var carry_value = BigInteger.Zero;

        for (int i = bytes.Length - 1; i >=0; i--)            // Walk the bytes from right to left.
        {
            var shifted_value = (BigInteger)bytes[i] << shiftBits;
